    ...
    if (_formKey.currentState?.validate() ?? false) {
      try {
        ...
        if (user != null) {
          ...
          // Retrieve stored password hash or comparison data if applicable
          String? storedPasswordHash = data?['password']; // Replace with actual field if needed

          // Example of how to check and update password (this part may be more complex based on your requirements)
          if (_passwordController.text != storedPasswordHash) {
            // Update password using Firebase Authentication
            await user.updatePassword(_passwordController.text);
            
            // Optionally, update Firestore with a new password hash (avoid storing plaintext passwords)
            await _firestore.collection('users').doc(user.uid).update({
              'password': _passwordController.text, // Replace with a hashed password if needed
            });
          }
