...
class TestDetailsPage extends StatelessWidget {
  ...
  Future<void> _deleteExistingSubmission() async {
    try {
      ...
      QuerySnapshot existingSubmissionSnapshot = await FirebaseFirestore.instance
          .collection('studentTestAnswer')
          .where('studentID', isEqualTo: userID)
          .where('testID', isEqualTo: testID)
          .get();

      for (var doc in existingSubmissionSnapshot.docs) {
      ...
      throw Exception('Failed to delete existing submission: $e');
    }
  }
  ...
  void _submitAnswers(BuildContext context, List<Map<String, String>> answers) async {
    ...
    try {
      ...
      // Create a unique ID for the new submission document
